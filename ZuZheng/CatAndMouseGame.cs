...
public class CatAndMouseGame
{
    ...
    static CatAndMouseGame()
    {
        ...
        for (var i = 0; i < bytes.Length / 4; i++)
            if (i % 2 == 0)
            {
                stageData[i / 2 * 4] = bytes[i * 4];
                stageData[i / 2 * 4 + 1] = bytes[i * 4 + 1];
                stageData[i / 2 * 4 + 2] = bytes[i * 4 + 2];
                stageData[i / 2 * 4 + 3] = bytes[i * 4 + 3];
            }
            else
            {
                stageTop[i / 2 * 4] = bytes[i * 4];
                stageTop[i / 2 * 4 + 1] = bytes[i * 4 + 1];
                stageTop[i / 2 * 4 + 2] = bytes[i * 4 + 2];
                stageTop[i / 2 * 4 + 3] = bytes[i * 4 + 3];
            }
    ...
    public static string MouseGame3WithKeys(string str, string key)
    {
        byte[] info;
        byte[] home;
        var output = "";
        OtherHomeBuilding(key, out home, out info);
        output = MouseGame3KeysHome(str, home, info);
        Console.WriteLine("KeyOutput:" + Encoding.UTF8.GetString(home).TrimEnd(new char[1]));
        return output + "\r\ninfoKeys:" + Encoding.UTF8.GetString(info).TrimEnd(new char[1]);
    }

    public static string MouseGame3KeysHome(string str, byte[] home, byte[] info)
    {
    ...
    public static string CatHome(byte[] data, byte[] home, byte[] info, bool isCompress = false)
    {
        ...
        if (array != null) return Convert.ToBase64String(array);
        return null;
    }
    ...
    private class DataDecryptor : IDisposable
    {
        ...
        public byte[] ToByteArray()
        {
            if (!isCompress && memoryStream != null)
                return memoryStream.Capacity != memoryStream.Length ? memoryStream.ToArray() : memoryStream.GetBuffer();
